...
def clean_alert_text(text: str) -> str:
    """Remove Telegram message chrome accidentally included in DOM text."""
    cleaned = " ".join(str(text or "").split()).strip()
    if re.fullmatch(r"\d{1,3}\s+\d{1,2}:\d{2}", cleaned):
        return ""
    # Telegram Web appends view count + browser-local display time to the
    # message bubble (e.g. ``77 02:48``).  It is not alert content.
    cleaned = re.sub(r"\s+\d{1,3}\s+\d{1,2}:\d{2}\s*$", "", cleaned).strip()
    return cleaned
...
def _extract_name(raw: str) -> str:
    ...
    # Some Telegram DOM variants omit the emoji but retain the heading and
    # count on the same line.  Remove that heading before the HK code.
    code_match = re.search(r"\(\s*HK\.?\s*\d{1,5}\s*\)", raw, re.I)
    if not code_match:
        return ""
